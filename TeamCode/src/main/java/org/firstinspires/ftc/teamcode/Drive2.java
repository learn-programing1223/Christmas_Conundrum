    ...
    public void runOpMode() throws InterruptedException{
        ...
        while(opModeIsActive()) {
            ...
            if(gamepad2.left_trigger > 0.1){
                ...
                if(targetPos + (gamepad2.left_trigger*increment) >= 200){
                    targetPos=200;
                }
                else{
                    targetPos += (gamepad2.left_trigger*increment);
                }
                bot.fourBar.setTargetPosition(Math.round(targetPos));
            }
                ...
                if(targetPos - (gamepad2.right_trigger*increment) <= 0){
                    targetPos=0;
                }
                else{
                    targetPos -= (gamepad2.right_trigger*increment);
                }
                bot.fourBar.setTargetPosition(Math.round(targetPos));
            }
            ...
            else if (gamepad2.dpad_down && !lateDown){
                bot.fourBar.setPower(0.2);
                targetPos=0;
                bot.fourBar.setTargetPosition(Math.round(targetPos));
            ...
            else if (gamepad2.dpad_up && !lateUp){
                reset = false;
                if(!bot.clawClosed){
                    bot.claw();
                }
            ...
            //middle of tree
            else if (gamepad2.dpad_left && !lateLeft){
                reset = false;
                if(!bot.clawClosed){
                    bot.claw();
                }
            ...
            else if (gamepad2.dpad_right && !lateRight){
                reset = false;
                if(!bot.clawClosed){
                    bot.claw();
                }
            ...
            lateDown=gamepad2.dpad_down;
            lateUp=gamepad2.dpad_up;
            lateLeft=gamepad2.dpad_left;
            lateRight=gamepad2.dpad_right;
